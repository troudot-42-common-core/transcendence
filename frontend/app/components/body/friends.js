...
const renderAcceptedFriendships = async (div, data) => {
  ...
  for (let i = 0; i < requests.length; i++) {
    const friend_username =
      requests[i].user_username === user_username
        ? requests[i].friend_username
...
export const friends = async (render, div) => {
  ...
  await renderPendingFriendships(
    document.getElementById('pendingFriendsRow'),
    data,
  );
  await renderRequestedFriendships(
    document.getElementById('requestedFriendsRow'),
    data,
  );
  await renderAcceptedFriendships(document.getElementById('friendsRow'), data);

  const buttonSelection = Object.values(
    document.getElementsByClassName('button'),
  );
  buttonSelection.forEach((button) => {
    button.addEventListener('click', async () => {
      if (
        button.classList.contains('accept') ||
        button.classList.contains('decline')
      ) {
        const friendUsername = button.getAttribute('data-friend-username');
        await patchFriendship(
          friendUsername,
          button.classList.contains('accept') ? 'accept' : 'decline',
        );
        await reload();
  ...
  document
    .getElementById('toAddFriendButton')
