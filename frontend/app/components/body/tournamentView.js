...
const joinTournament = async (tournament_name) => {
  const response = await loggedFetch(fetch)(
    `/api/tournament/${tournament_name}/`,
    {
      method: 'PUT',
      credentials: 'include',
      headers: {
  ...
  for (const game of tournament.rows[rowId - 1].games) {
    ...
    for (const friendInfo of game.players) {
      ...
      col.innerHTML = `
                ...
                <a href="/user/${friendInfo.username}/" id="pendingFriendName" data-link>
                    ${truncate(friendInfo.display_name, 8)}
                </a>
            `;
    ...
    // Players' scores
    if (
      (game.status === 'finished' || game.status === 'saving') &&
      game.winner !== null
    ) {
      ...
      for (let j = 0; j < game.scores.length; j++) {
        ...
        if (game.winner === game.players[j].username) {
          col.style =
            'color: green; font-weight: bold; font-size: larger; padding: 5px;';
        } else {
          col.style =
            'color: red; font-weight: bold; font-size: larger; padding: 5px;';
        }
        if (
          game.scores.some((score) => score.player === game.players[j].username)
        ) {
          col.innerHTML = game.scores.find(
            (score) => score.player === game.players[j].username,
          ).score;
        }
...
const onClickStagesTab = (tournament, event, data, username) => {
  const id =
    event.target.getAttribute('data-tabs-id') ||
    event.target.firstElementChild.getAttribute('data-tabs-id');
  ...
  document.querySelectorAll('.tabs').forEach((element) => {
    if (
      tournament.rows[element.getAttribute('data-tabs-id') - 1] === undefined
    ) {
...
const renderPlayersOfTournament = (div, data, tournament, username) => {
  ...
  const user_in_players = players.some(
    (player) => player.username === username,
  );
  const nb_players =
    tournament.nb_of_players > players.length && !user_in_players
      ? players.length + 1
      : players.length;
...
export const tournamentView = async (render, div, args) => {
  ...
  if (!tournament) {
    return (window.location.href = '/tournament/');
  }
  const websocket = websocketsHandler.getWs('tournamentView');
  ...
  renderPlayersOfTournament(
    div.querySelector('#playersRow'),
    data,
    tournament,
    username,
  );
  ...
  renderStagesTabsOfTournament(
    div.querySelector('#stageTabList'),
    data,
    tournament,
    username,
  );
